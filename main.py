...
async def set_bot_commands(bot: Bot):
    ...
    commands = [
        ...
        BotCommand(command="getmodel", description="🔍 Текущая модель LLM"),
        BotCommand(command="settemp", description="🌡️ Настроить температуру"),
...
async def handle_health_check(request):
    """Эндпоинт для проверки работоспособности (Render, UptimeRobot)."""
    return web.Response(text="Telegram Bot is alive 24/7! 🚀", status=200)


async def start_web_health_server():
    """Запускает мини-веб-сервер для поддержания активности на облачном хостинге."""
...
async def main():
    """Главный цикл с автоперезапуском при сбоях."""
    retry_delay = 5
    while True:
        try:
